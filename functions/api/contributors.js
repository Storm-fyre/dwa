...
export async function onRequestGet(context) {
  try {
    const { results } = await context.env.DB.prepare("SELECT * FROM contributors ORDER BY created_at DESC").all();
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    await context.env.DB.prepare("INSERT INTO contributors (name, amount) VALUES (?, ?)").bind(data.name, data.amount).run();
...
export async function onRequestDelete(context) {
  ...
  try {
    ...
    await context.env.DB.prepare("DELETE FROM contributors WHERE id = ?").bind(id).run();
    return new Response(JSON.stringify({ success: true }));
  } catch (error) { return new Response(JSON.stringify({ error: error.message }), { status: 500 }); }
}